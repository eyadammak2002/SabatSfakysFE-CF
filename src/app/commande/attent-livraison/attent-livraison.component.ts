...
@Component({
  selector: 'app-attent-livraison',
  standalone: true,
  imports: [FormsModule, CommonModule, NgClass, RouterModule],
  templateUrl: './attent-livraison.component.html',
  styleUrls: ['./attent-livraison.component.css']
})
export class AttentLivraisonComponent implements OnInit {
  commandes: Panier[] = [];
  userId!: number;
  userEmail!: string;
    ...
    this.panierService.getFournisseurByEmail(this.userEmail).pipe(
      ...
      switchMap(fournisseur => {
        if (fournisseur) {
          ...
          return this.panierService.getCommandesByFournisseurCOMMANDEE(this.fournisseurId);
        } else {
